...
type Order struct {
	gorm.Model
	Quantity         int		`valid:"required~ระบุจำนวน,range(1|100)~จำนวนอยู่ในช่วง (1-100)"` // 12
	Prices           float64	`valid:"required~ระบุราคา,range(1|1000000)~ราคาอยู่ในช่วง (1-1000000)"` //1
	Shelving_ID      *uint
	Shelving         Shelving	
