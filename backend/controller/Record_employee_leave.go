...
func ListRecord_employee_leave(c *gin.Context) {
	var record_employee_leave []entity.Record_employee_leave
	if err := entity.DB().Raw("SELECT * FROM record_employee_leaves").Scan(&record_employee_leave).Error; err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func UpdateRecord_employee_leave(c *gin.Context) {
	...
	if tx := entity.DB().Where("id = ?", record_employee_leave.ID).First(&record_employee_leave); tx.RowsAffected == 0 {
		c.JSON(http.StatusBadRequest, gin.H{"error": "cart not found"})
		return
	}

	if err := entity.DB().Save(&record_employee_leave).Error; err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
